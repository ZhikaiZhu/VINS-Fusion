...
void Estimator::extract_nonlinear_factors(MarginalizationInfo* marginalization_info, std::vector<long>& keyframes, std::unordered_map<long, int>& keyframe_addr_to_idx)
{
    ...
    double* res_rel_pos =  new double[3];
    double** jaco_rel_pos = new double* [2];
    jaco_rel_pos[0] = new double[3 * 7];
    jaco_rel_pos[1] = new double[3 * 7];

    double* res_rel_yaw = new double[1];
    double** jaco_rel_yaw = new double* [2];
    jaco_rel_yaw[0] = new double[1 * 7];
    jaco_rel_yaw[1] = new double[1 * 7];

    double* res_rel_rp = new double[2];
    double** jaco_rel_rp = new double* [2];
    jaco_rel_rp[0] = new double[2 * 7]; 
    jaco_rel_rp[1] = new double[2 * 7];
    ...
    for (auto kf: keyframes)
    {
        if (kf != keyframe_to_marg)
        {
            ...
            std::shared_ptr<RelPositionFactor> relPosF = std::make_shared<RelPositionFactor>(P_w_ij, Eigen::MatrixXd::Identity(3, 3));
            Eigen::Map<Eigen::Matrix<double, 3, 7, Eigen::RowMajor>> d_rel_pos_d_T_i(jaco_rel_pos[0]);
            Eigen::Map<Eigen::Matrix<double, 3, 7, Eigen::RowMajor>> d_rel_pos_d_T_j(jaco_rel_pos[1]);
            relPosF->Evaluate(parameters, res_rel_pos, jaco_rel_pos);
            J.block<3, 6>(0, marginalization_info->parameter_block_idx_aux[keyframe_to_marg] - marginalization_info->m_aux) = d_rel_pos_d_T_i.leftCols(6);
            J.block<3, 6>(0, marginalization_info->parameter_block_idx_aux[kf] - marginalization_info->m_aux) = d_rel_pos_d_T_j.leftCols(6);

            Eigen::Vector3d z_rel_yaw = Q_ij.inverse() * Eigen::Vector3d::UnitX();
            std::shared_ptr<RelYawFactor> relYawF = std::make_shared<RelYawFactor>(z_rel_yaw, Eigen::MatrixXd::Identity(1, 1));
            Eigen::Map<Eigen::Matrix<double, 1, 7, Eigen::RowMajor>> d_rel_yaw_d_T_i(jaco_rel_yaw[0]);
            Eigen::Map<Eigen::Matrix<double, 1, 7, Eigen::RowMajor>> d_rel_yaw_d_T_j(jaco_rel_yaw[1]);
            relYawF->Evaluate(parameters, res_rel_yaw, jaco_rel_yaw);
            J.block<1, 6>(3, marginalization_info->parameter_block_idx_aux[keyframe_to_marg] - marginalization_info->m_aux) = d_rel_yaw_d_T_i.leftCols(6);
            J.block<1, 6>(3, marginalization_info->parameter_block_idx_aux[kf] - marginalization_info->m_aux) = d_rel_yaw_d_T_j.leftCols(6);

            std::shared_ptr<RelRollPitchFactor> relRPF = std::make_shared<RelRollPitchFactor>(Q_ij, Eigen::MatrixXd::Identity(2, 2));
            Eigen::Map<Eigen::Matrix<double, 2, 7, Eigen::RowMajor>> d_rel_rp_d_T_i(jaco_rel_rp[0]);
            Eigen::Map<Eigen::Matrix<double, 2, 7, Eigen::RowMajor>> d_rel_rp_d_T_j(jaco_rel_rp[1]);
            rollPitchF->Evaluate(parameters, res_rel_rp, jaco_rel_rp);
            J.block<2, 6>(4, marginalization_info->parameter_block_idx_aux[keyframe_to_marg] - marginalization_info->m_aux) = d_rel_rp_d_T_i.leftCols(6);
            J.block<2, 6>(4, marginalization_info->parameter_block_idx_aux[kf] - marginalization_info->m_aux) = d_rel_rp_d_T_j.leftCols(6);
            Eigen::MatrixXd cov_new = J * marginalization_info->cov_old * J.transpose();
            ...
            rel_odom.pose.pose.position.x = P_w_ij.x();
            rel_odom.pose.pose.position.y = P_w_ij.y();
            rel_odom.pose.pose.position.z = P_w_ij.z();
    ...
    delete[] jaco_rel; */
    delete[] res_rel_pos;
    delete[] jaco_rel_pos[0];
    delete[] jaco_rel_pos[1];
    delete[] jaco_rel_pos;
    delete[] res_rel_yaw;
    delete[] jaco_rel_yaw[0];
    delete[] jaco_rel_yaw[1];
    delete[] jaco_rel_yaw;
    delete[] res_rel_rp;
    delete[] jaco_rel_rp[0];
    delete[] jaco_rel_rp[1];
    delete[] jaco_rel_rp;
    // delete[] parameters[0];
    // delete[] parameters[1];
    delete[] parameters;
