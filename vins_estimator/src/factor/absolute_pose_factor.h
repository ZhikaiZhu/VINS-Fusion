...
{
public:
    AbsPositionFactor(const Eigen::Vector3d& _z_pos, const Eigen::Matrix<double, 3, 3>& _sqrt_info): z_pos(_z_pos), sqrt_info(_sqrt_info) {}
...
{
public:
    RollPitchFactor(const Eigen::Quaterniond& _z_rp, const Eigen::Matrix<double, 2, 2>& _sqrt_info): z_rp(_z_rp), sqrt_info(_sqrt_info) {}
    ...
    void check_consistency(double** parameters)
    {
	    double* res = new double[2];
		double** jaco = new double* [1];
	    jaco[0] = new double[2 * 7];
    ...
    virtual bool Evaluate(double const* const* parameters, double* residuals, double** jacobians) const
    {
        ...
        if (jacobians)
        {
            if (jacobians[0])
            {
                ...
                Eigen::Matrix<double, 1, 3> proj;
                proj << 0.0, 1.0, 0.0;
                jacobian_orientation_i.block<1, 3>(0, 3) = sqrt_info * proj * jacobian_orientation;
            }
        }
        return true;
    }
